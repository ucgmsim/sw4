...
\begin{itemize}
	...
	\item Receivers must be placed in the {\em interior} of the computational domain, outside the
	      super-grid layers, for the reason spelled out in Section~\ref{sec:supergrid}: the seismic wave
	      equation is modified inside the layers, so a time history recorded there is not a ground
	      motion prediction. \emph{SW4} checks this at start-up and prints one warning line per
	      offending receiver plus a summary line, and records how far each receiver is inside the layer
	      in the output itself --- {\tt USER8}/{\tt USER9} in SAC (Section~\ref{sec:sac-format}),
	      {\tt SGDEPTH}/{\tt SGDEPTHGP} in HDF5 (Section~\ref{sec:sachdf5-format}), and a
	      {\tt \# WARNING:} header line in the ASCII text format. A value of zero means the receiver is
	      in the interior. A source in the layer is a harder error and stops the run; see
	      {\tt allowsourceinsupergrid} in Section~\ref{keyword:developer}.
...
\begin{flushleft}\bf
	...
	\tt
	developer
	cfl=... checkfornan=... allowsourceinsupergrid=... \\
	\bf Required parameters:\\
	\rm
	None
\end{flushleft}
...
\begin{center}
	\begin{tabular}{|l|p{10cm}|l|l|} \hline
		...
		checkfornan & Scan solution arrays for NaN & string    & off          \\ \hline
		allowsourceinsupergrid & Run anyway when a source lies inside the
		super-grid layer, or within five grid points of it, instead of stopping.
		The full diagnostic is still printed, as a warning. The resulting
		simulation is {\em not} a ground motion prediction: inside the layer
		\emph{SW4} solves a damped, coordinate-stretched equation, so the source
		does not radiate the moment it was given.
		            & string    & off
		\\ \hline
	\end{tabular}
\end{center}
\index{developer parameters! cfl, checkfornan, allowsourceinsupergrid}
...
When the simulation uses super-grid boundary conditions, \emph{SW4} records how far the receiver is
inside the super-grid absorbing layer in three otherwise unused header fields:

\begin{center}
	\begin{tabular}{|l|p{10cm}|} \hline
		\bf{Field} & \bf{Meaning}                                                        \\ \hline \hline
		USER8      & Distance into the super-grid layer, in meters. Zero means the receiver
		is in the interior, i.e. the time history is a ground motion prediction.
		\\ \hline
		USER9      & The same distance in grid points, rounded up.                         \\ \hline
		KUSER0     & The string {\tt SGLAYER}, marking {\tt USER8}/{\tt USER9} as carrying
		this quantity.
		\\ \hline
	\end{tabular}
\end{center}

The distance is clamped at zero and is never a signed clearance from the layer, because SAC uses
$-12345$ as its ``undefined'' value and a clearance of $-12345$ meters would be indistinguishable
from a field that was never set. The three fields are left undefined when the simulation has no
super-grid boundary, so {\tt USER8} $=-12345$ means ``not reported'', which is not the same as zero.
...
When the simulation uses super-grid boundary conditions, two further root-group attributes describe
the absorbing layer, so that the file describes its own geometry without reference to the input file:
{\tt SGWIDTH}, the width of the layer in meters, and {\tt SGWIDTHGP}, the same width in grid points of
the coarsest grid. Each station group then carries {\tt SGDEPTH}, how far that station is inside the
layer in meters, and {\tt SGDEPTHGP}, the same distance in grid points. A value of zero means the
station is in the interior, i.e. its time history is a ground motion prediction; any positive value
means it is inside the layer, where \emph{SW4} solves a damped, coordinate-stretched equation and the
time history is {\em not} a ground motion prediction. The natural severity measure is therefore
{\tt SGDEPTH}/{\tt SGWIDTH}, which runs from just above 0 at the inner edge of the layer to 1 at the
boundary.
...
\begin{center}
	\begin{tabular}{lll}\hline
		...
		SGWIDTH              & H5T\_STD\_F64LE  & 1           \\ \hline
		SGWIDTHGP            & H5T\_STD\_F64LE  & 1           \\ \hline
		...
		SGDEPTH              & H5T\_STD\_F64LE  & 1           \\ \hline
		SGDEPTHGP            & H5T\_STD\_F64LE  & 1           \\ \hline
