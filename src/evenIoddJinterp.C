...
// un-roll c-loop
//      for( int c=1 ; c <= 2 ; c++ ) // updated components 1,2 of the ghost point value of Uf
//      {
      int c=1;
...
// NOTE: Uc is not changed so all Uc-terms could be pre-computed
      b1 = UnextcInterp(c,i,j,1) + nuc*a_ghcof[0]*i16*(   -Uc(c,ic-1,jc,0)*Morc(ic-1,jc,1)+ 
							  ...
							  -Uc(c,ic+2,jc,0)*Morc(ic+2,jc,1)) - Unextf(c,i,j,nkf); 
...
// right hand side is mismatch in displacement                
      b1 = UnextcInterp(3,i,j,1) + nuc*a_ghcof[0]*i16*(   -Uc(3,ic-1,jc,0)*Mlrc(ic-1,jc,1)+ 
				       ...
				       -Uc(3,ic+2,jc,0)*Mlrc(ic+2,jc,1))  - Unextf(3,i,j,nkf); 
...
// Double-precision variant of evenIoddJinterpJacobiOpt (see cf_interface.h).
