...
EW::EW(const string &fileName, vector<vector<Source *>> &a_GlobalSources,
    ...
    : //  m_epi_lat(0.0), m_epi_lon(0.0), m_epi_depth(0.0), m_epi_t0(0.0),
      ...
      // achievable, but in single precision it sits at FLT_EPSILON (~1.19e-7),
      // so the Jacobi iteration could never reach it and would exhaust
      // m_cimaxiter. Use a looser default (~840*FLT_EPSILON) when float_sw4 is
      // single precision.
      m_citol(sizeof(float_sw4) == 4 ? 1e-4 : 1e-7), m_cimaxiter(20),
