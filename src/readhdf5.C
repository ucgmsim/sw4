...
static herr_t traverse_func(hid_t loc_id, const char *grp_name,
  ...
  if (infobuf.type == H5O_TYPE_GROUP) {
    ...
    if (H5Lexists(grp, "USEZVALUE", H5P_DEFAULT) > 0) {
      ...
      ASSERT(attr > 0);
      ret = H5Dread(attr, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT,
                    &usezvalue);
      ASSERT(ret >= 0);
      H5Dclose(attr);
      if (usezvalue != 0)
    ...
    if (H5Lexists(grp, "WINDOWS", H5P_DEFAULT) > 0) {
      ...
      if (dset < 0)
        fprintf(stderr,
                "Error reading from rechdf5 station %s WINDOWS open failed!\n",
                grp_name);
      ret =
    ...
    if (!((inCurvilinear || z >= 0) && x >= 0 && x <= op_data->m_global_xmax &&
      ...
      if (ts_ptr->myPoint()) {
        ...
        if (op_data->is_obs) {
          ...
          if (foundwins)
            ts_ptr->set_window(op_data->win1, op_data->win2, op_data->win3,
                               op_data->win4);
...
static herr_t collect_names_cb(hid_t loc_id, const char *grp_name,
                               const H5L_info_t *info, void *operator_data) {
...
static bool read_one_station(hid_t file_id, const string &name, EW *ew,
  ...
  hid_t grp;
  H5E_BEGIN_TRY { grp = H5Gopen(file_id, name.c_str(), H5P_DEFAULT); }
  H5E_END_TRY;
  if (grp < 0)
  ...
  if (!((inCurvilinear || z >= 0) && x >= 0 && x <= gxmax && y >= 0 &&
        y <= gymax)) {
    ...
    if (ew->getVerbosity() > 0) {
      ...
      receivererr << endl
                  ...
                  << endl
                  << " WARNING:  RECEIVER positioned outside grid!" << endl
                  << " No RECEIVER file will be generated for station = "
                  << name << endl;
      if (geoCoordSet)
...
void readStationHDF5(EW *ew, string inFileName, string outFileName,
  ...
  if (!is_obs) {
    ...
    // whole file (~10 HDF5 metadata reads per station), i.e. O(stations x
    // ranks) uncoordinated filesystem traffic, plus 2 collective MPI_Allreduce
    // per station inside the TimeSeries constructor, i.e. O(stations)
    // serialized collectives. Both dominate startup at scale. This replacement
    // is:
    //
    ...
    if (rank == 0) {
      ...
      if (fid < 0)
        ...
        H5Literate(fid, H5_INDEX_NAME, H5_ITER_NATIVE, NULL, collect_names_cb,
                   &names);
        H5Fclose(fid);
      }
      H5Pclose(fapl);
    ...
    if (rank != 0) {
      ...
      int pos = 0;
      for (int s = 0; s < ncand; s++) {
        names[s] = string(&namebytes[pos]);
        pos += (int)names[s].size() + 1;
      }
    }
    ...
    // metadata server; spread them evenly across the job (every 'stride'-th
    // rank) so the readers land on as many different nodes as possible.
    int target_readers = nprocs < 128 ? nprocs : 128;
    if (target_readers > ncand)
      target_readers = ncand;
    if (target_readers < 1)
      target_readers = 1;
    ...
    if (am_reader) {
      ...
      if (fid < 0)
        ...
        for (int s = begin; s < end; s++) {
          sta_rec_t rec;
          if (read_one_station(fid, names[s], ew, m_global_xmax, m_global_ymax,
                               rec)) {
            lcoord.push_back(rec.x);
            lcoord.push_back(rec.y);
            lcoord.push_back(rec.z);
            lflag.push_back(rec.nsew);
            lflag.push_back(rec.topodepth);
            lnames += names[s];
            lnames += '\0';
          }
        }
    ...
    if (rank == 0) {
      printf("readStationHDF5: %d reader ranks read %d in-domain stations in "
             "%.2f s\n",
    ...
    for (int p = 0; p < nprocs; p++) {
      ...
      cdispls[p] = 3 * displs[p];
    }
    vector<double> gcoord(3 * total);
    MPI_Allgatherv(lcoord.data(), 3 * local_n, MPI_DOUBLE, gcoord.data(),
                   ccounts.data(), cdispls.data(), MPI_DOUBLE, comm);
    // flags: 2 ints per station
    vector<int> fcounts(nprocs), fdispls(nprocs);
    for (int p = 0; p < nprocs; p++) {
      fcounts[p] = 2 * counts[p];
      fdispls[p] = 2 * displs[p];
    }
    vector<int> gflag(2 * total);
    MPI_Allgatherv(lflag.data(), 2 * local_n, MPI_INT, gflag.data(),
                   fcounts.data(), fdispls.data(), MPI_INT, comm);
    ...
    MPI_Allgatherv(lnames.data(), lnbytes, MPI_CHAR, gnames.data(),
                   ncounts.data(), ndispls.data(), MPI_CHAR, comm);
    vector<string> snames(total);
    {
      int pos = 0;
      for (int s = 0; s < total; s++) {
        snames[s] = string(&gnames[pos]);
        pos += (int)snames[s].size() + 1;
      }
    }

    // ---- 3a. batched topography elevation: one Allreduce over all stations
    // ---- Each rank interpolates topography locally for every station
    // (returning a sentinel where the (x,y) column isn't in its subdomain); one
    // MPI_MAX reduction then yields the true elevation, replacing the
    // per-station Allreduce the TimeSeries ctor would otherwise do.
    vector<double> zTopo(total, 0.0);
    if (ew->topographyExists()) {
      vector<float_sw4> zloc(total), zglob(total);
      for (int s = 0; s < total; s++) {
        float_sw4 zt;
        if (!ew->m_gridGenerator->interpolate_topography(
                ew, gcoord[3 * s], gcoord[3 * s + 1], zt, ew->mTopoGridExt))
          zt = -1e38;
        zloc[s] = zt;
      }
      MPI_Allreduce(zloc.data(), zglob.data(), total, ew->m_mpifloat, MPI_MAX,
                    comm);
      for (int s = 0; s < total; s++)
        zTopo[s] = zglob[s];
    }
    ...
    for (int s = 0; s < total; s++) {
      TimeSeries *ts_ptr =
          new TimeSeries(ew, snames[s], snames[s], mode, false, false, true,
                         outFileName, gcoord[3 * s], gcoord[3 * s + 1],
                         gcoord[3 * s + 2], gflag[2 * s + 1] != 0, writeEvery,
                         downSample, gflag[2 * s] == 0, event,
      ...
      if (rank == 0) {
        ...
        for (int s = 0; s < total; s++)
          if (ownsum[s] != 1)
            nbad++;
        if (nbad > 0)
          printf("readStationHDF5: WARNING, %d of %d stations are not owned by "
                 "exactly one rank\n",
                 nbad, total);
      }
    }
    ...
    if (rank == 0) {
      printf("readStationHDF5: constructed %d TimeSeries in %.2f s (total "
             "station setup %.2f s)\n",
...
void readRuptureHDF5(char *fname,
  ...
  for (int pts = 0; pts < npts; pts++) {
    ...
    vs = (double)point_data[pts].vs;
    den = (double)point_data[pts].den;
    ...
    if (world_rank == 0 && mVerbose >= 3) {
      printf("point #%i: lon=%g, lat=%g, dep=%g, stk=%g, dip=%g, area=%g, "
             "tinit=%g, dt=%g, vs=%g, den=%g\n",
             pts + 1, lon, lat, dep, stk, dip, area, tinit, dt, vs, den);
      printf("          rake=%g, slip1=%g, nt1=%i, slip2=%g, nt2=%i, slip3=%g, "
             "nt3=%i\n",
    ...
    if (nt1 > 0) {
      ...
      if (world_rank == 0 && mVerbose >= 3) {
        printf("INFO: SRF file: dt*sum(slip_vel)=%e [m], total slip (from "
               "header)=%e [m]\n",
               slip_sum, slip_m);
      }
      float_sw4 slip_sum_tol =
          sizeof(float_sw4) == sizeof(float) ? 1e-4 : 1e-12;
      ...
      if (slip_sum > -slip_sum_tol && slip_sum < slip_sum_tol) {
        ...
        if (world_rank == 0 && nskip_zero_slip <= 10) {
          printf("WARNING: skipping rupture point #%i because "
                 "dt*sum(slip_vel)=%e [m], total slip (from header)=%e [m]\n",
      ...
      // we convert [cm] to [m]
      double vs_m = 1e-2 * vs;        // = vs is in cm/s
      double den_kgm3 = 1000.0 * den; // den is g/cm^3 = 1000 kg/m^3
      double mu = den_kgm3 * vs_m * vs_m;
      m0 = area * 1e-4 * slip1 * 1e-2 * mu;
      ...
      if (x < xmin || x > m_global_xmax || y < ymin || y > m_global_ymax ||
        ...
        sourcePtr = new Source(
            ew, freq, t0, x, y, z, mxx, mxy, mxz, myy, myz, mzz, tDep,
            formstring, topodepth, ncyc, par, npar, ipar, nipar,
            false); // shear modulus now accounts for the SRF local shear
                    // modulus so there is no requirement for correction.
  ...
  if (world_rank == 0)
    printf("Read npts=%i, made %i point moment tensor sources, nu1=%i, nu2=%i, "
           "nu3=%i\n",
           npts, nSources, nu1, nu2, nu3);
  if (world_rank == 0 && nskip_zero_slip > 0)
    printf(
        "Skipped %i rupture points with zero slip-velocity integral in u1.\n",
        nskip_zero_slip);
