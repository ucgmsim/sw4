...
#pragma omp for reduction(max:lowz)
	 ...
	 for( int j=jfirst+2; j <= jlast-2 ; j++ )
	    for( int i=ifirst+2; i <= ilast-2 ; i++ )
	    {
	       float_sw4 er=std::abs(rho(i,j,k)*uacc(c,i,j,k)-lu(c,i,j,k)-fo(c,i,j,k));
	       if( lowz < er )
		  lowz = er;
	    }
      lowZ[c-1] = lowz;
...
#define alpham(c,i,j,k) a_alpham[base3+i+ni*(j)+nij*(k)+nijk*(c)]
...
#pragma omp simd
	 ...
	 for( int i=ifirst+2 ; i<=ilast-2 ; i++ )
	 {
            float_sw4 r1 = (-cm*alpham(1,i,j,k-kl)+(4+omdt*omdt)*i6*u(1,i,j,k-kl)+
			    i6*(1-omdt)*um(1,i,j,k-kl)+ memforce(1,i,j) )/cp;
            float_sw4 r2 = (-cm*alpham(2,i,j,k-kl)+(4+omdt*omdt)*i6*u(2,i,j,k-kl)+
			    i6*(1-omdt)*um(2,i,j,k-kl)+ memforce(2,i,j) )/cp;
            float_sw4 r3 = (-cm*alpham(3,i,j,k-kl)+(4+omdt*omdt)*i6*u(3,i,j,k-kl)+
...
// normal derivatives
	    float_sw4 un1 = s[1]*alphap(1,i,j,k)+s[2]*alphap(1,i,j,k+kl)
               +s[3]*alphap(1,i,j,k+2*kl)
               +s[4]*alphap(1,i,j,k+3*kl) + s[0]*r1;
	    float_sw4 vn1 = s[1]*alphap(2,i,j,k)+s[2]*alphap(2,i,j,k+kl)
               +s[3]*alphap(2,i,j,k+2*kl)
               +s[4]*alphap(2,i,j,k+3*kl) + s[0]*r2;
	    float_sw4 wn1 = s[1]*alphap(3,i,j,k)+s[2]*alphap(3,i,j,k+kl)
