...
#pragma omp parallel for 
	    for( int j=jb+2 ; j <= je-2 ; j++ )
		  ...
		  size_t ind = i-ib+ni*(j-jb)+nij*(k-kb);
		  float_sw4 wx = d4a*(u[2*npts+ind+1]-u[2*npts+ind-1])+d4b*(u[2*npts+ind+2]-u[2*npts+ind-2]);
		  float_sw4 ux = d4a*(u[ind+1]-u[ind-1])+d4b*(u[ind+2]-u[ind-2]);
		  float_sw4 wy = d4a*(u[2*npts+ind+ni  ]-u[2*npts+ind-ni  ])+
					     d4b*(u[2*npts+ind+2*ni]-u[2*npts+ind-2*ni]);
		  float_sw4 vy = d4a*(u[npts+ind+ni]  -u[npts+ind-ni])+
...
//-----------------------------------------------------------------------
void EW::twdirbdryc_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
			...
			float_sw4* z )
{
   ...
   for( int k=wind[4]; k <= wind[5] ; k++ ) {
	 ...
	 for( int i=wind[0]; i <= wind[1] ; i++ ) {
	    ...
	    bforce[  3*qq]=sin(om*(x[ind]-cv*t))*sin(om*y[ind]+ph)*sin(om*z[ind]+ph);
	    bforce[1+3*qq]=sin(om*x[ind]+ph)*sin(om*(y[ind]-cv*t))*sin(om*z[ind]+ph);
	    bforce[2+3*qq]=sin(om*x[ind]+ph)*sin(om*y[ind]+ph)*sin(om*(z[ind]-cv*t));
	    qq++;
	 }}}
}

//-----------------------------------------------------------------------
void EW::twfrsurfz_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
	 ...
	 double forces[3];
	 double t13,t15,t16,t19,t20,t21,t23,t24,t28,t29,t32,t33,t34,t37,t38,t43,t44,t49,t60,t62,t65;
...
//-----------------------------------------------------------------------
void EW::twfrsurfzatt_ci(int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
			 float_sw4 h, int kz, float_sw4 t, float_sw4 omega, float_sw4 c,
...
//-----------------------------------------------------------------------
void EW::twfrsurfzsgstr_ci(int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
	 ...
	 size_t qq  = (i-ifirst)+ni*(j-jfirst);
	 double x=(i-1)*h, y=(j-1)*h; 
	 double t1,t10,t11,t12,t15,t17,t19,t20,t22,t24,t25,t29,t3,t31,t32,t36,t39,t4,t40,t46,t51,t53,t56,t6,t7;
	 double forces[3];
...
//-----------------------------------------------------------------------
void EW::twfrsurfzsgstratt_ci(int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
			   float_sw4 h, int kz, float_sw4 t, float_sw4 omega, float_sw4 c,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
	 ...
	 t22 = omega*y+phase;
	 t23 = cos(t22);
	 forces[0] = mua[ind]*(t5*t8*t13*omega+t19*t8*omega*t23*t13);
	 t28 = sin(t3);
	 ...
	 t43 = cos(t7);
	 t45 = sin(t22);
	 forces[1] = mua[ind]*(-t28*t32*t36*omega+t42*t43*t45*omega*t13);
	 t51 = cos(t12);
	 t56 = sin(t4);
	 t61 = omega*t51;
	 t66 = sin(t31);
...
//-----------------------------------------------------------------------
void EW::twstensor_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
      for( int i=ifirst ; i<= ilast ; i++ )
      {
	 double t1,t11,t12,t20,t21,t23,t24,t26,t3,t30,t31,t35,t36,t37,t38,t4,t41,t42,t46,t50,t51,t54,t7,t8;
	 size_t ind = (i-ifirst)+ni*(j-jfirst)+nij*(kz-kfirst);
	 size_t qq  = (i-ifirst)+ni*(j-jfirst);
...
//-----------------------------------------------------------------------
void EW::twstensorsg_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
			 int kz, float_sw4 t, float_sw4 om, float_sw4 c, float_sw4 ph, 
			 float_sw4* xx, float_sw4* yy, float_sw4* zz,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
	 ...
	 double t12,t13,t14,t16,t17,t18,t2,t24,t26,t28,t29,t30,t32,t33,t35,t39,t4,t40,t44,t45;
...
//-----------------------------------------------------------------------
void EW::twstensoratt_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
	 ...
	 double x=xx[ind], y=yy[ind], z=zz[ind];
	 double muu=mu[ind], lambdaa=lambda[ind];
	 double t1,t12,t13,t15,t16,t17,t19,t20,t24,t27,t28,t3,t31,t32,t36,t37,t4,t41,t42,t44,t48,t49,t5,t61,t64,t8,t9;
	 double stensor[6];
...
//-----------------------------------------------------------------------
void EW::twstensorsgatt_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst, int klast,
			    int kz, float_sw4 t, float_sw4 omega, float_sw4 c, float_sw4 phase, 
...
#pragma omp parallel for
   for( int j=jfirst ; j<= jlast ; j++ )
      for( int i=ifirst ; i<= ilast ; i++ )
      {
	 double t10,t13,t14,t17,t18,t2,t21,t22,t24,t26,t31,t33,t34,t38,t39,t4,t42,t43,t44;
	 ...
	 t59 = sin(t17);
	 stensor[2] = muu*(t21*t14*t59*omega+t4*t14*omega*t48*t59);
