...
void EW::randomfield3d_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst,
	       ...
	       for( int kk=-pz ; kk <= pz ; kk++ )
                        ...
                        if( j-jj >= jfirst  &&  j-jj <= jlast )
			{
			   for( int ii=-p; ii <= p; ii++ )
			      ...
			      {
				 w(i-ii,j-jj,k-kk) += b(ii)*b(jj)*bz(kk)*randno;
				 wgh(i-ii,j-jj,k-kk) += b(ii)*b(jj)*bz(kk)*b(ii)*b(jj)*bz(kk);
			      }
			}
		     }
	    }
...
//-----------------------------------------------------------------------
void EW::randomfield3dc_ci( int ifirst, int ilast, int jfirst, int jlast, int kfirst,
			    int klast, int nig, int njg, int nkg, int gh, float_sw4* __restrict__ a_w,
			    float_sw4* __restrict__ a_wgh, float_sw4 dist, float_sw4 distz, float_sw4 h,
	       ...
	       for( int kk=-pz ; kk <= pz ; kk++ )
                        ...
                        if( j-jj >= jfirst  &&  j-jj <= jlast )
			{
			   for( int ii=-p; ii <= p; ii++ )
			      if( i-ii >= ifirst  &&  i-ii <= ilast )
			      {
                                 float_sw4 bz = exp(-(z(i-ii,j-jj,k-kk)-z(i-ii,j-jj,k))*
						    (z(i-ii,j-jj,k-kk)-z(i-ii,j-jj,k))*i2dz);
...
//-----------------------------------------------------------------------
void EW::perturbvelocity_ci(int ifirst, int ilast, int jfirst, int jlast, int kfirst,
